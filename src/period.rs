...
pub fn fpw(
    ...
    freqs.iter().map(|&freq| {
        fpw_statistic(t, &ivar, &ivar_y, freq, n_bins)
    }).collect()
}
...
pub fn freq_grid(
    ...
    let trange = t.iter().cloned().fold(f64::NAN, f64::max) - t.iter().cloned().fold(f64::NAN, f64::min);
...
pub fn get_best_freq(
    freqs: &[f64],
    result: &[f64],
) -> (f64, f64) {
    let max_index = result.iter()
        .enumerate()
        .filter(|&(_, &x)| !x.is_nan())
...
pub fn get_best_freqs(
    freqs: &[f64],
    result: &[f64],
    n: usize
) -> Vec<(f64, f64)> {
    let mut freq_stat_pairs: Vec<(f64, f64)> = freqs.iter()
        .zip(result.iter())
        .filter(|&(_, &x)| !x.is_nan())
        .map(|(&f, &s)| (f, s))
        .collect();
